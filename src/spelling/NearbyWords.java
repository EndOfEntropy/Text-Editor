	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		*/
  		while(!queue.isEmpty() && retList.size() < numSuggestions && count < THRESHOLD) {
  			String curr = queue.get(0).toLowerCase();
  			queue.remove(word);
  			List<String> stemList = new LinkedList<String>();
  			stemList = distanceOne(curr, true);
  			
  			for(String n: stemList) {
